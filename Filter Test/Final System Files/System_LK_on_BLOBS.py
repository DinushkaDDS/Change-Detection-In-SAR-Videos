...
def _main_():
    ...
    pointsInterest_ref = np.array([[[0, 0]]], dtype=np.float32)
    radiusInterest_ref = np.array([[[0, 0]]], dtype=np.float32)
    ...
    while videoFile.isOpened():
        ...
        if ret:
            ...
            blobPoints = np.array([[[0, 0]]], dtype=np.float32)
            for point in keypoints:
                blobPoints = np.append(blobPoints, [[[np.float32(point[0]),
                                                      np.float32(point[1])]]], axis=0)
            blobPoints = np.delete(blobPoints, 0, axis=0)

            points_curr, statusArr, err = cv2.calcOpticalFlowPyrLK(
                frameRef_gray, frameCurr_gray, blobPoints, None, **lkparams
            )

            len_points_curr = len(points_curr)
            i = 0
            while (i< len_points_curr):
                if(statusArr[i]==1):
                    new = points_curr[i][0]
                    old = blobPoints[i][0]
                    a, b = new.ravel()
                    ...
                    len_points_curr = len_points_curr - 1
                    points_curr = np.delete(points_curr, i, axis=0)
                    blobPoints = np.delete(blobPoints, i, axis=0)

            pointsInterest_curr, statusInterestArr, err = cv2.calcOpticalFlowPyrLK(
                frameRef_gray, frameCurr_gray, pointsInterest_ref, None, **lkparams
            )

            len_point_interest = len(pointsInterest_curr)
            i = 0
            while(i < len_point_interest):

                if(statusInterestArr[i]==1):
                    new = pointsInterest_curr[i][0]
                    old = pointsInterest_ref[i][0]
                    a, b = new.ravel()
                    c, d = old.ravel()
                    mask = cv2.line(mask, (a, b), (c, d), [0, 255, 0], 1)
                    frameCurr = cv2.circle(frameCurr, (a, b), 5, [255, 0, 0], -1)
                    ...
                    len_point_interest = len_point_interest -1
                    pointsInterest_curr = np.delete(pointsInterest_curr, i, axis=0)
                    pointsInterest_ref = np.delete(pointsInterest_ref, i, axis=0)

            # Need to remove the nearer points to avoid duplication and computational complexity
            pointsInterest_curr = np.concatenate((pointsInterest_curr, points_curr), axis=0)
            pointsInterest_ref = pointsInterest_curr
